    ...
    */
    ...
    public static double computePagIbig(double grossSalary){
          if (grossSalary >= 1000 && grossSalary <= 1500) {
            return grossSalary * 0.01; // 1% for employee
            } 
          else if (grossSalary > 1500) {
            return grossSalary * 0.02; // 2% for employee
            } 
          else {
            return 0; // if salary below 1000
            } 
        }
    ...
    * The tax amount is calculated using progressive tax brackets
    ...
    public static double computeIncomeTax(double monthlyGross) {
        ...
        if (monthlyGross <= 20832) {
            ...
            return (monthlyGross - 20833) * 0.20; // 20% in excess of 20,833
            }   
        else if (monthlyGross <= 66666) {
            return 2500 + (monthlyGross - 33333) * 0.25; // 2,500 + 25% in excess of 33,333
            } 
        else if (monthlyGross <= 166666) {
            return 10833 + (monthlyGross - 66667) * 0.30; // 10,833 + 30% in excess of 66,667
            } 
        else if (monthlyGross <= 666666) {
            return 40833.33 + (monthlyGross - 166667) * 0.32; // 40,833.33 + 32% in excess over 166,667
            } 
        else {
            return 200833.33 + (monthlyGross - 666667) * 0.35; // 200,833.33 + 35% in excess of 666,667
            } 
            }
    ...
    * @param args command-line arguments
    ...
    try (BufferedReader br = new BufferedReader(new FileReader(employeeDatabase))) {
     ...
     while ((line = br.readLine()) != null) {
        ...
        if (!validnumber) {
            ...
            System.out.println("");
            System.out.println("Employee Number: " + displayNumber);
    ...
    while ((line = br.readLine()) != null) {
        ...
        if (year != 2024 || recordMonth != month) continue;
            ...
            double totalHoursWorked = computeTotalHoursWorked(login, logout);

        if (day <= 15) 
            firstHalf += totalHoursWorked;
        
        else
            secondHalf += totalHoursWorked;
            }
    ...
    try (BufferedReader attendanceReader = new BufferedReader(new FileReader(attendanceRecords))) {
    ...
    while ((attendanceLine = attendanceReader.readLine()) != null) {
        ...
        if (attendanceLine.trim().isEmpty()) continue;
        ...
        if (year != 2024 || recordMonth != month) continue;
            ...
            LocalTime login = LocalTime.parse(attendanceData[4].trim(), timeFormat);
            LocalTime logout = LocalTime.parse(attendanceData[5].trim(), timeFormat);
         ...
         if (day <= 15)
            ...
            int index = month - 6;
                ...
                System.out.println("Payroll calculation successful.");
